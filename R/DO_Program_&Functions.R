...
#' }
#'
#' @note This program relies heavily on dialog boxes generated by the
#'   `svDialogs` package. Default measures are in place to ensure baseline
#'   functionality across operating systems and R interfaces. However, a better
#'   experience may be achievable by passing additional arguments to the dialog
#'   functions via `...`, as the vignette for `Observation` describes.
#'
#' @seealso
#' [example_data]
data_collection_program_interactive <- function(...){
...
#' if (interactive()) {
#'
#' # compendium_reference_interactive() is called within
#' # compendium_reference() when `interactive = TRUE` and
